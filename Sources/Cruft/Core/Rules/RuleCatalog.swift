...
enum RuleCatalog {
    ...
    private static let node: [Rule] = [
        Rule(
            ...
            item: ItemInfo(
                description: "`node_modules/` holds the dependency tree installed by npm, pnpm, yarn, or bun from the registry. It accumulates per project and is fully reproducible from a lockfile.",
                safetyNote: "A lockfile is present, so `npm install` (or the matching package manager) restores the exact same tree.",
        ...
        Rule(
            ...
            languageKey: "javascript",
            item: ItemInfo(
                description: "A `node_modules/` next to a `package.json` with no lockfile (`package-lock.json`, `pnpm-lock.yaml`, `yarn.lock`, `bun.lock`, `bun.lockb`). Reinstalling resolves against current registry state and may pull different transitive versions than were installed.",
                safetyNote: "Reinstall produces a *similar* tree but not necessarily the same one — new patch/minor versions may be pulled in. Consider generating a lockfile before deleting.",
        ...
        Rule(
            ...
            toolKey: "nextjs",
            item: ItemInfo(
                description: "`.next/` is Next.js's build output: compiled server and client bundles, static assets, and incremental build cache. Created by `next build` and also populated during `next dev`.",
                safetyNote: "Reproducible by running `next build`; the dev server recreates it on startup.",
        ...
        Rule(
            ...
            toolKey: "nuxt",
            item: ItemInfo(
                description: "`.nuxt/` is generated by `nuxt dev` and `nuxt build`. It contains the compiled Vue application, generated routes, server middleware, and TypeScript type definitions derived from the project structure.",
                safetyNote: "Regenerated on every dev server start and build run.",
        ...
        Rule(
            ...
            toolKey: "sveltekit",
            item: ItemInfo(
                description: "`.svelte-kit/` is SvelteKit's internal output directory, written during `vite dev` and `vite build`. It contains generated route manifests, TypeScript configuration, adapter output, and compiled server/client code.",
                safetyNote: "Regenerated on every dev server start and build run.",
        ...
        Rule(
            ...
            toolKey: "turborepo",
            item: ItemInfo(
                description: "`.turbo/` stores Turborepo's local task cache: hashed inputs and outputs per pipeline task. When inputs haven't changed, Turborepo replays the cached output instead of re-running.",
                safetyNote: "Tasks re-run without cache hits on the next invocation; no source files are affected.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "parcel",
            item: ItemInfo(
                description: "`.parcel-cache/` stores the results of every file transformation and bundle step from previous Parcel builds, so unchanged work can be skipped on rebuild.",
                safetyNote: "Triggers a full cold rebuild on the next `parcel` invocation; no source files are modified.",
        ...
        Rule(
            ...
            toolKey: "vite",
            item: ItemInfo(
                description: "Vite stores pre-bundled dependencies in `node_modules/.vite/` (and a legacy `.vite/` location) to speed dev server startup. CommonJS and UMD packages are converted to ESM and bundled so the browser makes fewer requests.",
                safetyNote: "Regenerated on the next `vite dev` startup or when lockfile/config changes are detected.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "astro",
            item: ItemInfo(
                description: "`.astro/` holds Astro's auto-generated TypeScript type definitions (`.astro/types.d.ts`, referenced from `tsconfig.json`) and internal build metadata. The build cache itself defaults to `node_modules/.astro/`.",
                safetyNote: "Regenerated when the Astro dev server starts or a build runs.",
        ...
        Rule(
            ...
            toolKey: "npm",
            item: ItemInfo(
                description: "`~/.npm/_cacache` is npm's content-addressable store of downloaded package tarballs and metadata, keyed by integrity hash. Shared across all projects so previously downloaded packages skip the network.",
                safetyNote: "npm re-populates the cache on the next `npm install`.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "yarn",
            item: ItemInfo(
                description: "`~/Library/Caches/Yarn` (classic) or `~/.yarn/berry/cache` (Berry/v2+) holds compressed package archives reused across all projects so previously downloaded packages skip the network.",
                safetyNote: "Yarn re-downloads and revalidates packages from the registry on the next install.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "pnpm",
            item: ItemInfo(
                description: "`~/Library/pnpm/store` keeps a single copy of every package version, with each project's `node_modules` entries hardlinked into it so identical packages are never duplicated. Direct deletion would orphan every linked project; `pnpm store prune` removes only versions no live project references.",
        ...
        Rule(
            ...
            toolKey: "bun",
            item: ItemInfo(
                description: "`~/.bun/install/cache` stores downloaded packages as `<name>@<version>` subdirectories. On macOS, Bun uses `clonefile` from the cache into `node_modules` so package data is shared rather than duplicated.",
                safetyNote: "Bun re-downloads and re-extracts packages on the next `bun install`.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "nvm",
            item: ItemInfo(
                description: "`~/.nvm/versions/node/` holds each Node.js version installed by nvm as a self-contained directory with the Node binary, bundled npm, and standard library for that release.",
                safetyNote: "Removing a version only affects projects pinned to it; reinstall with `nvm install <version>`.",
        ...
        Rule(
            ...
            toolKey: "fnm",
            item: ItemInfo(
                description: "`~/.fnm/node-versions/` holds each Node.js version installed by fnm as a self-contained directory containing the complete Node runtime for that release.",
        ...
        Rule(
            ...
            toolKey: "nvm",
            item: ItemInfo(
                description: "`~/.local/share/nvm` (or a custom `$nvm_data` path) holds each Node.js version installed by nvm.fish as a self-contained directory per release.",
        ...
        Rule(
            ...
            toolKey: "asdf",
            item: ItemInfo(
                description: "`~/.asdf/installs/nodejs/` holds each Node.js version installed via the `asdf-nodejs` plugin, one self-contained directory per version.",
        ...
        Rule(
            ...
            toolKey: "n",
            item: ItemInfo(
                description: "`~/n/versions/node/` (or `$N_PREFIX/n/versions/node/`) caches each Node.js version `n` has downloaded, one directory per release, so switching between them requires no re-download.",
        ...
        Rule(
            ...
            toolKey: "deno",
            item: ItemInfo(
                description: "`~/Library/Caches/deno` is Deno's combined cache: `deps/` (ESM modules from URL imports), `gen/` (TypeScript→JavaScript compile cache), `npm/` (npm-compatible package store), and Deno KV files. Override via `$DENO_DIR`.",
    ...
    private static let apple: [Rule] = [
        Rule(
            ...
            toolKey: "xcode",
            item: ItemInfo(
                description: "`~/Library/Developer/Xcode/DerivedData` holds per-project build intermediates: compiled object files, index data, Swift module caches, and linked binaries. Xcode creates one UUID-named subdirectory per project.",
        ...
        Rule(
            ...
            toolKey: "xcode",
            item: ItemInfo(
                description: "`~/Library/Developer/Xcode/iOS DeviceSupport` stores symbol files and debugging support data fetched from a physical device the first time it is connected. Each iOS version occupies its own subdirectory.",
        ...
        Rule(
            ...
            toolKey: "xcode",
            item: ItemInfo(
                description: "`~/Library/Developer/Xcode/Archives` stores `.xcarchive` bundles produced by **Product > Archive**. Each contains the compiled app binary, dSYM symbol files, and metadata for App Store submission and crash log symbolication.",
        ...
        Rule(
            ...
            toolKey: "swiftpm",
            item: ItemInfo(
                description: "`.build/` is Swift Package Manager's build directory, created alongside `Package.swift`. It contains compiled object files, linked executables, and resolved dependency checkouts under `.build/checkouts/`.",
        ...
        Rule(
            ...
            toolKey: "cocoapods",
            item: ItemInfo(
                description: "`Pods/` contains downloaded source code for every pod declared in `Podfile`, plus generated Xcode projects the workspace references. Created and managed by `pod install`.",
        ...
        Rule(
            ...
            toolKey: "carthage",
            item: ItemInfo(
                description: "`Carthage/Build/` holds compiled `.xcframework` or `.framework` bundles produced by Carthage and linked directly into the Xcode project. `Cartfile.resolved` pins every framework version.",
        ...
        Rule(
            ...
            toolKey: "cocoapods",
            item: ItemInfo(
                description: "`~/Library/Caches/CocoaPods` stores pod source archives and spec files, avoiding re-downloads of the same pod version across multiple projects.",
        ...
        Rule(
            ...
            toolKey: "xcode",
            item: ItemInfo(
                description: "`~/Library/Developer/CoreSimulator/Devices` stores data and state for every iOS, watchOS, tvOS, and visionOS simulator created in Xcode. Simulators whose runtime has been uninstalled remain on disk marked \"unavailable\".",
                safetyNote: "`xcrun simctl delete unavailable` removes only orphaned simulators; active simulators and their data are untouched.",
        ...
        Rule(
            ...
            tier: .low, aggregation: .none,
            notes: "Orphaned simulator OS runtime bundles.",
            iconAsset: "xcode",
            brandTint: xcodeBlue,
            ...
            toolKey: "xcode",
            item: ItemInfo(
                description: "`/Library/Developer/CoreSimulator/Profiles/Runtimes` holds simulator runtime bundles installed by Xcode — one per OS version and platform (iOS, watchOS, tvOS, visionOS).",
        ...
        Rule(
            ...
            notes: "Kernel Debug Kits for kext debugging.",
            iconAsset: "xcode",
            brandTint: xcodeBlue,
            languageKey: "swift",
            toolKey: "xcode",
            item: ItemInfo(
                description: "Kernel Debug Kits (KDKs) at `/Library/Developer/KDKs` contain symbols and headers for kernel-extension (kext) and kernel-level debugging. Each KDK targets a specific macOS kernel version.",
        ...
        Rule(
            ...
            toolKey: "xcode",
            item: ItemInfo(
                description: "`~/Library/Developer/CoreSimulator/Caches` holds transient disk images and runtime assets pre-warmed by CoreSimulator to speed simulator boot.",
                safetyNote: "CoreSimulator recreates these images on the next simulator launch; app data and installed simulator runtimes are unaffected.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            item: ItemInfo(
                ...
                safetyNote: "Rebuilt by Xcode on the next compile of each project; the first build after clearing is slower while the cache repopulates.",
                regenCommand: nil,
                links: [
                    InfoLink(title: "Xcode — build system", url: "https://developer.apple.com/documentation/xcode/build-system", kind: .docs),
                ]
        ...
        Rule(
            ...
            toolKey: "xcode",
            item: ItemInfo(
                description: "`DerivedData/CompilationCache.noindex` is the explicit-modules / cached-compilation store added in Xcode 16. It speeds builds by reusing compiled outputs across targets that share the same input fingerprint.",
                safetyNote: "Rebuilt by Xcode; subsequent compiles are slower until the cache repopulates.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            item: ItemInfo(
                ...
                safetyNote: "Rebuilt on the next preview render. Previews take an extra moment to come up the first time after clearing.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            item: ItemInfo(
                ...
                safetyNote: "Recreated on the next simulator boot; diagnostic-only and doesn't affect simulator state.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "xcode",
            item: ItemInfo(
                description: "`~/Library/Caches/com.apple.dt.Xcode/fsCachedData` is Xcode's generic NSURLCache-style storage — Asset Catalog thumbnails, account avatars, remotely-fetched resources, and small artifacts. The sibling `Cache.db*` SQLite files follow the same lifecycle.",
        ...
        Rule(
            ...
            toolKey: "swiftpm",
            item: ItemInfo(
                description: "`~/Library/Caches/org.swift.swiftpm` is SwiftPM CLI's user cache: `manifests/`, `repositories/` (cloned dependency repos), `package-collection.db`, `package-metadata/`. Used by `swift build` and Xcode — distinct from `xcode.swiftpm-cache`, which targets Xcode's per-IDE SwiftPM directory.",
        ...
        Rule(
            ...
            toolKey: "cocoapods",
            item: ItemInfo(
                description: "`~/Library/Caches/CocoaPods` is the user-level cache of downloaded pod source archives (`Pods/Cache/`) plus reusable working copies. Distinct from `~/.cocoapods` (master spec repo) and the per-project `Pods/` directory — equivalent to `pod cache clean --all`.",
                safetyNote: "Re-downloaded from the source repo (often GitHub) on the next `pod install`.",
                regenCommand: "pod install",
                links: [
        ...
        Rule(
            ...
            item: ItemInfo(
                ...
                safetyNote: "`ReportCrash` writes a fresh report on the next crash; these are diagnostic records of past failures, not anything macOS depends on.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            languageKey: "swift",
            item: ItemInfo(
                description: "`~/Library/Logs/Spin Reports` stores spindump samples macOS captures when a user process becomes unresponsive. Common with debug GUI builds that hang under the debugger.",
                safetyNote: "macOS writes a fresh report on the next hang; samples are diagnostic-only.",
                regenCommand: nil,
                links: [
    ...
    private static let rust: [Rule] = [
        ...
        Rule(
            ...
            toolKey: "rustup",
            item: ItemInfo(
                description: "`~/.rustup/toolchains` stores each installed Rust toolchain (`stable`, `beta`, `nightly`, version-pinned) as a self-contained directory containing `rustc`, `cargo`, the standard library, and other components.",
        ...
        Rule(
            ...
            toolKey: "sccache",
            item: ItemInfo(
                description: "`~/Library/Caches/Mozilla.sccache` is sccache's local on-disk cache of hashed compile artifacts. Despite the Rust association, sccache works for C, C++, and CUDA too. Capped by `SCCACHE_CACHE_SIZE`.",
    ...
    private static let python: [Rule] = [
        Rule(
            ...
            languageKey: "python",
            item: ItemInfo(
                description: "`.venv/` is a project-local Python virtual environment created by `python -m venv .venv`, `uv venv`, or `poetry install`. It holds an isolated interpreter, pip, and all installed packages.",
        ...
        Rule(
            ...
            languageKey: "python",
            item: ItemInfo(
                description: "`venv/` is a project-local Python virtual environment created by `python -m venv venv`. Same structure as `.venv/` — isolated interpreter, pip, and installed packages.",
                safetyNote: "Reproducible from `requirements.txt`, `Pipfile`, or `pyproject.toml`.",
                regenCommand: "python -m venv venv && pip install -r requirements.txt",
                links: [
                    InfoLink(title: "venv — Python docs", url: "https://docs.python.org/3/library/venv.html", kind: .docs),
        ...
        Rule(
            ...
            languageKey: "python",
            item: ItemInfo(
                description: "`__pycache__/` holds compiled bytecode (`.pyc`) that CPython writes alongside each `.py` source file on first import. One directory per package in the tree.",
                safetyNote: "Python regenerates bytecode on the next import.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "pip",
            item: ItemInfo(
                description: "`~/Library/Caches/pip` stores downloaded wheel archives and HTTP responses. pip checks here before hitting the network, so reinstalling a previously downloaded version skips the network.",
        ...
        Rule(
            ...
            toolKey: "uv",
            item: ItemInfo(
                description: "`~/Library/Caches/uv` stores downloaded wheel archives in a content-addressed store shared across all uv-managed environments, deduplicating identical package versions.",
        ...
        Rule(
            ...
            toolKey: "poetry",
            item: ItemInfo(
                description: "`~/Library/Caches/pypoetry` holds wheel archives and package metadata fetched during `poetry install`, shared across all Poetry-managed projects (override via `poetry config cache-dir`).",
                safetyNote: "Poetry re-downloads required packages from PyPI on the next `poetry install`.",
        ...
        Rule(
            ...
            toolKey: "pyenv",
            item: ItemInfo(
                description: "`~/.pyenv/versions/` holds each Python interpreter installed by pyenv. Each subdirectory is a self-contained installation with its own interpreter binary, standard library, and pip.",
        ...
        Rule(
            ...
            toolKey: "pipenv",
            item: ItemInfo(
                description: "`~/.cache/pipenv` holds pipenv's HTTP wheel cache, hash cache, and cached packages. Known for unbounded growth (issue #3655). Override with `PIPENV_CACHE_DIR`.",
        ...
        Rule(
            ...
            toolKey: "pdm",
            item: ItemInfo(
                description: "`~/.cache/pdm` holds PDM's HTTP cache, downloaded wheels, package metadata, and hash cache. PDM uses XDG-style paths even on macOS.",
        ...
        Rule(
            ...
            toolKey: "conda",
            item: ItemInfo(
                description: "`~/.conda/pkgs` stores downloaded `.conda`/`.tar.bz2` package archives plus extracted package directories that serve as the link source for conda environments.",
    ...
    private static let go: [Rule] = [
        Rule(
            ...
            toolKey: "go-modules",
            item: ItemInfo(
                description: "`~/Library/Caches/go-build` is the Go build cache (`GOCACHE`), storing compiled package objects and test results keyed by their inputs. Only packages whose inputs change are recompiled.",
    ...
    private static let jvm: [Rule] = [
        ...
        Rule(
            ...
            toolKey: "gradle",
            item: ItemInfo(
                description: "`.gradle/` is Gradle's per-project cache: the Gradle wrapper JAR (`wrapper/dists/`), project-level caches, and configuration state. Separate from the global `~/.gradle` cache.",
        ...
        Rule(
            ...
            toolKey: "maven",
            item: ItemInfo(
                description: "`target/` is Maven's build output directory: compiled `.class` files, packaged JARs or WARs, generated sources, and test results produced by the Maven lifecycle.",
        ...
        Rule(
            ...
            toolKey: "gradle",
            item: ItemInfo(
                description: "`~/.gradle/caches` is Gradle's global cache: downloaded dependencies, resolved script classpath artifacts, the build cache, and distribution metadata shared across all projects.",
        ...
        Rule(
            ...
            toolKey: "maven",
            item: ItemInfo(
                description: "`~/.m2/repository` is Maven's local repository — a mirror of artifacts downloaded from Maven Central and other remote repositories. It grows with every distinct dependency version used across all projects.",
                safetyNote: "Maven re-downloads required artifacts from Maven Central or configured mirrors on the next build.",
        ...
        Rule(
            ...
            toolKey: "coursier",
            item: ItemInfo(
                description: "`~/Library/Caches/Coursier/v1` holds resolved JARs, POMs, and source artifacts shared by every Coursier-backed JVM tool — sbt, Mill, Bloop, Metals, Scala-CLI.",
        ...
        Rule(
            ...
            toolKey: "sbt",
            item: ItemInfo(
                description: "`~/.ivy2` is sbt's classic Ivy resolution cache (`cache/`, `local/`) — the older sibling of Coursier, still used by many sbt builds. Override with `sbt.ivy.home`.",
        ...
        Rule(
            ...
            toolKey: "sbt",
            item: ItemInfo(
                description: "`~/.sbt` holds sbt's launcher boot directories (`boot/` — one per Scala/sbt version pair) and minigraph dependency cache (`1.0/dependency/`).",
        ...
        Rule(
            ...
            toolKey: "scala-cli",
            item: ItemInfo(
                description: "`~/Library/Caches/ScalaCli` is Scala CLI's working state: Bloop server workdir, scripts cache, downloaded JVMs/Scala compilers. Distinct from Coursier (which Scala CLI also uses for dependency JARs).",
        ...
        Rule(
            ...
            toolKey: "gradle",
            item: ItemInfo(
                description: "`~/.gradle/wrapper/dists/` holds the full Gradle distribution (`gradle-X.Y.Z-bin.zip`) for every version any Gradle Wrapper has requested. Each project's `gradle/wrapper/gradle-wrapper.properties` pins one.",
        ...
        Rule(
            ...
            action: .trash, tier: .high, aggregation: .none,
            notes: "Per-API-level emulator system images.",
            iconAsset: "android",
            brandTint: Color(red: 0x3D/255, green: 0xDC/255, blue: 0x84/255),  // Android green
            languageKey: "java",
            toolKey: "android-sdk",
            item: ItemInfo(
                description: "`~/Library/Android/sdk/system-images/` holds emulator system images, one per API level + architecture. AVDs (Android Virtual Devices) reference them.",
        ...
        Rule(
            ...
            toolKey: "android-sdk",
            item: ItemInfo(
                description: "`~/Library/Android/sdk/.downloadIntermediates/` (and the sibling `.temp/`) hold partial and intermediate downloads from the Android SDK Manager.",
    ...
    private static let cpp: [Rule] = [
        Rule(
            ...
            toolKey: "cmake",
            item: ItemInfo(
                description: "`build/` is CMake's out-of-source build directory, created by `cmake -B build`. Contains generated Makefiles or Ninja scripts, compiled object files, linked libraries, executables, and `CMakeCache.txt`.",
        ...
        Rule(
            ...
            toolKey: "ccache",
            item: ItemInfo(
                description: "`~/Library/Caches/ccache` (or `~/.ccache` if configured) is ccache's cache of previous compiler outputs, keyed by preprocessed source and compiler flags. Supports GCC, Clang, and compatible compilers for C, C++, Objective-C, and CUDA.",
                safetyNote: "Causes slower rebuilds; ccache repopulates the cache transparently on the next compilation.",
                regenCommand: nil,
                links: [
    ...
    private static let bazel: [Rule] = [
        Rule(
            ...
            toolKey: "bazel",
            item: ItemInfo(
                description: "`~/.cache/bazel` is Bazel's output user root: the action cache, external repository downloads, and build outputs for every workspace on the machine.",
    ...
    private static let packageManagers: [Rule] = [
        Rule(
            ...
            toolKey: "homebrew",
            item: ItemInfo(
                description: "`~/Library/Caches/Homebrew` holds bottle tarballs (precompiled binaries) and formula source archives downloaded by `brew install` and `brew upgrade`. Homebrew checks it before hitting the network, so reinstalls of the same version skip the download.",
                safetyNote: "Re-downloaded on the next `brew install` or `brew upgrade`; installed formulae at `/opt/homebrew` are unaffected.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "macports",
            item: ItemInfo(
                description: "`~/.macports/` stores user-level MacPorts configuration, including per-user variant overrides that control how ports are compiled (enabling or disabling optional features).",
                safetyNote: "Only per-user variant preferences are lost; installed ports at `/opt/local` are unaffected and overrides can be re-applied with `port variant`.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "nix",
            item: ItemInfo(
                description: "`~/.cache/nix/` stores flake evaluation SQLite databases and tarballs fetched during `nix` command runs. The cache speeds up repeated evaluations of the same flake inputs.",
                safetyNote: "Rebuilt and re-fetched automatically on the next `nix` command; the Nix store at `/nix/store` is unaffected.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            item: ItemInfo(
                ...
                safetyNote: "Profile generation history (old rollback points) is lost; the Nix store and currently installed packages are not removed, but rollbacks older than the current generation become unavailable.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            item: ItemInfo(
                ...
                safetyNote: "`nix-collect-garbage -d` removes only unreferenced store paths; all packages reachable from live profiles remain intact.",
    ...
    private static let ruby: [Rule] = [
        ...
        Rule(
            ...
            toolKey: "rbenv",
            item: ItemInfo(
                description: "`~/.rbenv/versions/` contains each Ruby interpreter installed by rbenv (via the `ruby-build` plugin). Each subdirectory is a self-contained installation with the interpreter, standard library, and default gems.",
        ...
        Rule(
            ...
            toolKey: "rvm",
            item: ItemInfo(
                description: "`~/.rvm/rubies/` contains each Ruby interpreter managed by RVM. Each subdirectory is a self-contained installation with the interpreter binary, standard library, and default gems.",
    ...
    private static let haskell: [Rule] = [
        ...
        Rule(
            ...
            languageKey: "haskell",
            item: ItemInfo(
                description: "`~/.stack/` is Stack's global data directory, holding downloaded Stackage snapshots, prebuilt GHC installations, and compiled snapshot package stores shared across all Stack projects.",
                safetyNote: "Removes all Stack-managed GHC installations and compiled snapshot packages; the next `stack build` rebuilds from scratch and is slow.",
                regenCommand: "stack build",
                links: []
            )
        ),
        ...
        Rule(
            ...
            toolKey: "ghcup",
            item: ItemInfo(
                description: "`~/.ghcup/ghc/` contains GHC (Glasgow Haskell Compiler) versions installed by ghcup. Each subdirectory is a self-contained installation with the compiler, runtime libraries, and base packages.",
    ...
    private static let ai: [Rule] = [
        ...
        Rule(
            ...
            toolKey: "ollama",
            item: ItemInfo(
                description: "Ollama stores downloaded LLM weights under `~/.ollama/models/`. Blobs are content-addressed and shared between tags; running `ollama rm` decrements the reference count rather than leaving orphaned blob files.",
        ...
        Rule(
            ...
            toolKey: "lmstudio",
            item: ItemInfo(
                description: "LM Studio stores downloaded GGUF model files under `~/.lmstudio/models/<publisher>/<model>/`. Models are sourced from Hugging Face mirrors and can be browsed directly within the LM Studio app.",
        ...
        Rule(
            ...
            item: ItemInfo(
                description: "`~/jan/models/` holds Jan's downloaded model files. Each model directory contains the GGUF weights and metadata.",
        ...
        Rule(
            ...
            toolKey: "huggingface",
            item: ItemInfo(
                description: "The Hugging Face Hub Python library caches downloaded models, datasets, and spaces under `~/.cache/huggingface/hub/`. Each repository is stored as a content-addressed tree of blobs with symbolic-link snapshots, so multiple revisions share unchanged files.",
                safetyNote: "Large model re-downloads have substantial cost; use `huggingface-cli delete-cache` or the Python API to selectively remove only unneeded revisions.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "huggingface",
            item: ItemInfo(
                description: "The Hugging Face Xet storage layer adds a chunk-level deduplication cache at `~/.cache/huggingface/xet/`. It stores content-addressed data chunks and shard index files that speed up both downloads and uploads of Xet-enabled repositories.",
                safetyNote: "Pure optimisation layer; cached model files in `~/.cache/huggingface/hub/` are unaffected and `huggingface_hub` rebuilds the chunk cache transparently on subsequent access.",
                regenCommand: nil,
                links: [
                    InfoLink(title: "HuggingFace — manage cache", url: "https://huggingface.co/docs/huggingface_hub/guides/manage-cache", kind: .docs),
                ]
        ...
        Rule(
            ...
            toolKey: "pytorch",
            item: ItemInfo(
                description: "PyTorch Hub downloads pretrained model weights to `~/.cache/torch/hub/checkpoints/` when code calls `torch.hub.load()` or `torchvision.models.*()`. The directory also stores weights fetched via `torch.hub.download_url_to_file()`.",
                safetyNote: "Re-downloaded automatically the next time a model is loaded; large checkpoints can take a while to fetch.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "tensorflow",
            item: ItemInfo(
                description: "TensorFlow Datasets (TFDS) downloads and prepares datasets to `~/tensorflow_datasets/` via `tfds.load()`. Datasets are stored as sharded TFRecord files alongside metadata.",
                safetyNote: "Re-downloading and re-preparing large datasets has substantial cost; only delete datasets no longer needed for active projects.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "keras",
            item: ItemInfo(
                description: "Keras caches the pretrained weights of built-in application models (ResNet, EfficientNet, MobileNet, VGG, and others) under `~/.keras/models/`. Weights are downloaded automatically the first time a model is instantiated with `weights='imagenet'` or similar.",
        ...
        Rule(
            ...
            languageKey: "python",
            item: ItemInfo(
                description: "fastai downloads training datasets and pretrained model weights to `~/.fastai/data/`. Convenience functions such as `untar_data()` fetch standard benchmark datasets (ImageNette, MNIST, IMDB, etc.) and cache them here so subsequent runs skip the download.",
        ...
        Rule(
            ...
            toolKey: "ultralytics",
            item: ItemInfo(
                description: "Ultralytics YOLO automatically downloads pretrained model weights to `~/.config/Ultralytics/` on first use. The directory also stores settings and usage telemetry.",
        ...
        Rule(
            ...
            toolKey: "invokeai",
            item: ItemInfo(
                description: "InvokeAI stores Stable Diffusion and other image-generation model checkpoints under `~/invokeai/models/`. Models can be installed via the InvokeAI web UI or by placing checkpoint files directly into the directory.",
                safetyNote: "Large checkpoint re-downloads have substantial cost; reinstall via the InvokeAI model manager or Hugging Face.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            item: ItemInfo(
                description: "`~/.diffusionbee/` holds DiffusionBee's downloaded model checkpoints and app data. The directory also contains generated images if the default output path has not been changed.",
                safetyNote: "Removes all downloaded models and may remove locally generated images; re-download models from within the DiffusionBee app.",
                regenCommand: nil,
                links: []
            )
        ),
        ...
        Rule(
            ...
            toolKey: "playwright",
            item: ItemInfo(
                description: "Playwright downloads versioned browser binaries — Chromium, Firefox, and WebKit — to `~/Library/Caches/ms-playwright/` so test runs are hermetically isolated from system browsers. Multiple versions accumulate as Playwright is updated.",
                safetyNote: "Restored by `npx playwright install` or `playwright install`; the command only downloads browsers required by the current Playwright version.",
        ...
        Rule(
            ...
            toolKey: "cypress",
            item: ItemInfo(
                description: "Cypress downloads its bundled Electron-based browser binary to `~/Library/Caches/Cypress/` during installation, separate from the npm package itself. Multiple versions accumulate when projects pin different Cypress versions.",
        ...
        Rule(
            ...
            toolKey: "electron",
            item: ItemInfo(
                description: "When a project installs `electron` via npm, the postinstall script downloads the Electron runtime binary to `~/Library/Caches/electron/`. The download is shared across projects using the same Electron version, so the cache avoids redundant downloads.",
    ...
    private static let vm: [Rule] = [
        Rule(
            ...
            action: .trash, tier: .high, aggregation: .none,
            notes: "Docker Desktop sparse VM disk — all images, containers, and volumes.",
            iconAsset: "docker", brandTint: dockerBlue,
            toolKey: "docker",
            item: ItemInfo(
                description: "`Docker.raw` is the sparse disk image Docker Desktop uses as the entire storage backend for its Linux VM. All pulled images, containers, named volumes, and build cache live inside this file, which grows as data is added but does not shrink when data is removed.",
                safetyNote: "Permanently destroys all images, containers, and volumes — use `docker system prune -a --volumes` for targeted cleanup instead.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            action: .trash, tier: .high, aggregation: .none,
            notes: "Tart VM bundles — macOS and Linux disk images.",
            iconAsset: "", brandTint: Color(red: 0x10/255, green: 0x10/255, blue: 0x10/255),
            item: ItemInfo(
                description: "`~/.tart/vms/` stores macOS and Linux VM bundles used by Tart on Apple Silicon. Each bundle contains a disk image and a configuration file.",
                safetyNote: "Permanently destroys the VM's disk state — use `tart delete <name>` for a clean teardown, or re-pull with `tart pull <name>`.",
        ...
        Rule(
            ...
            iconAsset: "", brandTint: Color(red: 0x10/255, green: 0x10/255, blue: 0x10/255),
            item: ItemInfo(
                description: "`~/.tart/cache/` stores compressed OCI image layers downloaded by `tart pull`. Reused across pulls so the registry isn't hit repeatedly.",
                safetyNote: "Installed VMs in `~/.tart/vms/` are unaffected; only the layer cache for future pulls is removed.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            action: .trash, tier: .extreme, aggregation: .none,
            notes: "OrbStack sparse VM disk — all container images, volumes, k8s clusters, and Linux machines.",
            ...
            toolKey: "orbstack",
            item: ItemInfo(
                description: "`data.img.raw` is the sparse virtualization disk OrbStack uses as the storage backend for its container runtime, Kubernetes clusters, and Linux machines. The file's apparent size can look huge because it is sparse — actual on-disk usage is what matters.",
                safetyNote: "Permanently destroys every container image, volume, Kubernetes cluster, and Linux machine. Prefer `docker system prune -a --volumes` and `orb delete <machine>` for targeted cleanup.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            action: .trash, tier: .high, aggregation: .none,
            notes: "Downloaded Vagrant base box archives.",
            iconAsset: "vagrant", sfSymbol: "shippingbox.fill",
            brandTint: Color(red: 0x14/255, green: 0x63/255, blue: 0xFF/255),
            toolKey: "vagrant",
            item: ItemInfo(
                description: "`~/.vagrant.d/boxes/<provider>/<name>/<version>/` stores downloaded base VM box archives that `vagrant up` clones to create per-project VMs. Each box is a complete OS image, and multiple versions accumulate as boxes are upgraded.",
                safetyNote: "Already-cloned per-project VMs are unaffected. Boxes are re-fetched when a project requests a missing version, or via `vagrant box add`.",
        ...
        Rule(
            ...
            action: .trash, tier: .extreme, aggregation: .none,
            notes: "Per-project Vagrant VM state — destroys all in-VM data.",
            iconAsset: "vagrant", sfSymbol: "externaldrive.fill",
            brandTint: Color(red: 0x14/255, green: 0x63/255, blue: 0xFF/255),
            toolKey: "vagrant",
            item: ItemInfo(
                description: "`.vagrant/machines/<name>/<provider>/` holds the cloned VM disk, machine ID, and SSH key for each VM defined in this project's `Vagrantfile`.",
                safetyNote: "`vagrant up` re-provisions a fresh VM from the `Vagrantfile`, but files, packages, and state inside the old VM are gone. Run `vagrant destroy` first for a clean teardown that also removes the underlying provider VM.",
        ...
        Rule(
            ...
            action: .trash, tier: .extreme, aggregation: .none,
            notes: "Per-VM disk images and config — deletion is irreversible.",
            iconAsset: "virtualbox", sfSymbol: "externaldrive.fill",
            brandTint: Color(red: 0x18/255, green: 0x39/255, blue: 0x70/255),
            toolKey: "virtualbox",
            item: ItemInfo(
                description: "`~/VirtualBox VMs/<name>/` is a per-VM directory containing the `.vbox` config, `.vdi` / `.vmdk` disk images, snapshots, and logs. The disk image holds the installed OS, files, and configuration.",
                safetyNote: "Leaves a stale entry in VirtualBox's global registry (`~/Library/VirtualBox/VirtualBox.xml`). Run `VBoxManage unregistervm <name> --delete` for a clean removal that also clears the registry.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            action: .trash, tier: .extreme, aggregation: .none,
            notes: "UTM `.utm` VM bundle — disk image and config.",
            iconAsset: "utm", sfSymbol: "externaldrive.fill",
            brandTint: Color(red: 0x52/255, green: 0x2D/255, blue: 0xC4/255),
            toolKey: "utm",
            item: ItemInfo(
                description: "Each `.utm` bundle contains a VM's qcow2 disk(s), NVRAM/EFI variables, and `config.plist`. UTM stores these under its sandboxed Container directory.",
                safetyNote: "Trashing the bundle removes the VM permanently. UTM has no destructive CLI — recovering from the Trash is the only undo.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            action: .trash, tier: .extreme, aggregation: .none,
            notes: "`.vmwarevm` bundle — disk and snapshot state for a Fusion VM.",
            ...
            item: ItemInfo(
                description: "Each `.vmwarevm` bundle contains the VM's `.vmdk` disk(s), `.nvram`, `.vmx` config, and snapshots. The default location for newer Fusion installs is `~/Virtual Machines/`; older or upgraded installs may use `~/Documents/Virtual Machines.localized/`.",
                safetyNote: "Permanently deletes the VM. Fusion's library auto-removes stale entries on next launch.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            action: .trash, tier: .extreme, aggregation: .none,
            notes: "`.pvm` or `.macvm` bundle — disk and snapshot state.",
            iconAsset: "parallels", sfSymbol: "externaldrive.fill",
            brandTint: Color(red: 0xCC/255, green: 0x00/255, blue: 0x00/255),
            toolKey: "parallels",
            item: ItemInfo(
                description: "Each `.pvm` or `.macvm` bundle contains the guest's disk image, snapshots, NVRAM, and config. Parallels stores these in `~/Parallels/`; older versions and the App Store edition use different roots.",
                safetyNote: "Permanently deletes the VM. Parallels Control Center auto-removes stale library entries on next launch.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "lima",
            item: ItemInfo(
                description: "`~/.lima/<instance>/` holds the instance's `lima.yaml`, `disk` file, `cidata.iso`, and runtime sockets. `limactl start` rebuilds the disk from the YAML, but data inside the VM is destroyed on delete.",
                safetyNote: "Run `limactl delete <name>` first for a clean teardown that also stops the VM and removes its sockets.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "lima",
            item: ItemInfo(
                description: "`~/Library/Caches/lima/download/by-url-sha256/` stores downloaded cloud images (Ubuntu, Fedora, Alpine, …) keyed by source URL hash. Reused across instances to avoid redundant downloads.",
        ...
        Rule(
            ...
            action: .trash, tier: .extreme, aggregation: .none,
            notes: "Colima profile VM — all OCI images, containers, and named volumes.",
            iconAsset: "colima", sfSymbol: "externaldrive.fill",
            brandTint: Color(red: 0x2B/255, green: 0xB3/255, blue: 0x67/255),
            toolKey: "colima",
            item: ItemInfo(
                description: "`~/.colima/_lima/<profile>/` holds the Lima-managed VM backing Colima's container runtime. The disk contains every pulled OCI image, every container, and all named volumes.",
                safetyNote: "Destroys all container state. Prefer `colima delete [profile]` (or `colima delete --data` to wipe just the data disk) for a clean teardown.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "docker",
            item: ItemInfo(
                description: "`~/.docker/buildx/cache` is BuildKit's local content-addressable cache for `docker buildx build` invocations using the default builder. Distinct from Docker Desktop's VM-internal cache — this lives in the user's home directory.",
        ...
        Rule(
            ...
            toolKey: "docker",
            item: ItemInfo(
                description: "`~/.docker/scout` is Docker Scout's local cache of image vulnerability data. Grows with every image scanned by `docker scout cves`.",
        ...
        Rule(
            ...
            toolKey: "podman",
            item: ItemInfo(
                description: "`~/.local/share/containers/podman/machine/applehv/` holds the disk images backing each Podman machine on the Apple Hypervisor backend.",
                safetyNote: "`podman machine init` rebuilds the VM from scratch — re-downloads the base image and loses any container or image state held inside.",
        ...
        Rule(
            ...
            action: .trash, tier: .high, aggregation: .none,
            notes: "Rancher Desktop VM disk and k3s state.",
            iconAsset: "rancher",
            toolKey: "rancher-desktop",
            item: ItemInfo(
                description: "`~/Library/Application Support/rancher-desktop/` holds Rancher Desktop's VM disk image plus its embedded k3s/Kubernetes state.",
                safetyNote: "Reset via Preferences → Reset Kubernetes / Reset Application. Wipes any container or Kubernetes state held inside.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            action: .trash, tier: .high, aggregation: .none,
            notes: "Per-profile VM disks and downloaded ISOs/images.",
            iconAsset: "minikube",
            toolKey: "minikube",
            item: ItemInfo(
                description: "`~/.minikube/machines/` holds per-profile VM disks for minikube clusters. The sibling `~/.minikube/cache/` keeps downloaded ISOs and Kubernetes component images.",
        ...
        Rule(
            ...
            toolKey: "kind",
            item: ItemInfo(
                description: "`~/.local/share/kind/` caches kind's node images. Cluster state itself lives inside Docker — removing this cache only forces a re-pull of the node image on the next `kind create cluster`.",
    ...
    private static let aiCodingAgents: [Rule] = [
        ...
        Rule(
            ...
            action: .trash, tier: .extreme, aggregation: .none,
            notes: "Per-project Claude Code session transcripts.",
            ...
            toolKey: "claude-code",
            item: ItemInfo(
                description: "`~/.claude/projects/<encoded-cwd>/*.jsonl` contains the full transcript for every Claude Code conversation: model messages, tool calls, and diffs. Sessions accumulate per working directory.",
                safetyNote: "Transcripts are local-only and unrecoverable once deleted; `claude --resume` will no longer find them. Anthropic does not store these server-side.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "opencode",
            item: ItemInfo(
                description: "`~/.local/share/opencode/storage/` stores OpenCode (SST) session, message, and part records, organised per-project. Even on macOS, OpenCode follows the XDG Base Directory layout.",
                safetyNote: "Sessions are local-only and cannot be restored after deletion. Auth credentials in `~/.local/share/opencode/auth.json` are deliberately left untouched so you stay signed in.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "gemini-cli",
            item: ItemInfo(
                description: "`~/.gemini/tmp/<project_hash>/chats/session-*.json` stores per-project chat history; `~/.gemini/tmp/<project_hash>/checkpoints/` stores file checkpoints captured before edits.",
                safetyNote: "Chats are local-only and unrecoverable once deleted. Pre-edit checkpoints are usually also recoverable from git in a git repo.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            action: .trash, tier: .low, aggregation: .none,
            notes: "Aider repo-map ctags and treesitter cache.",
            iconAsset: "aider", sfSymbol: "bubble.and.pencil",
            brandTint: Color(red: 0xF6/255, green: 0x82/255, blue: 0x1F/255),  // Aider orange
            toolKey: "aider",
            item: ItemInfo(
                description: "Aider scans the repo with ctags and treesitter to build a repo map for the LLM, caching results under `.aider.tags.cache.v3/` next to the project.",
                safetyNote: "Rebuilt automatically on the next `aider` run in this project. Chat history (`.aider.chat.history.md`) is a separate file and is not touched.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "continue",
            item: ItemInfo(
                description: "`~/.continue/index/` holds Continue's persistent state for codebase chat: a LanceDB vector store with embeddings, an autocomplete cache, and a docs SQLite index.",
                safetyNote: "Re-indexed on the next workspace open.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "continue",
            item: ItemInfo(
                description: "`~/.continue/sessions/` stores Continue's chat history as one JSON per session. Sessions persist across IDE restarts.",
        ...
        Rule(
            ...
            toolKey: "continue",
            item: ItemInfo(
                description: "`~/.continue/.utils/` caches helper binaries Continue downloads on demand: a Chromium snapshot for the docs scraper plus an esbuild binary.",
        ...
        Rule(
            ...
            toolKey: "cline",
            item: ItemInfo(
                description: "`~/.cline/tasks/` stores Cline's per-task history: messages, tool inputs and outputs, and file diffs.",
                safetyNote: "Task histories are local-only and unrecoverable once deleted. Cline's checkpoint store (shadow git worktrees under VS Code's globalStorage) is not touched.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "goose",
            item: ItemInfo(
                description: "`~/.local/share/goose/sessions/` stores Goose CLI session transcripts as JSONL, one file per session. The companion config in `~/.config/goose/` is not touched.",
                safetyNote: "Sessions are local-only and unrecoverable once deleted.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            action: .trash, tier: .medium, aggregation: .none,
            notes: "Codeium daemon state, indexes, and completion models.",
            iconAsset: "windsurf", sfSymbol: "bubble.and.pencil",
            brandTint: Color(red: 0x14/255, green: 0xB1/255, blue: 0x9C/255),  // Codeium teal
            toolKey: "windsurf",
            item: ItemInfo(
                description: "`~/.codeium/windsurf/` is Windsurf's primary state directory: Codeium daemon state, codebase indexes, downloaded completion models, and per-project caches.",
                safetyNote: "Indexes and models are re-downloaded on next launch. Cascade chat history under `~/Library/Application Support/Windsurf/User/globalStorage/` is not touched.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            action: .trash, tier: .extreme, aggregation: .none,
            notes: "OpenHands conversation history, file_store, and sandbox state.",
            iconAsset: "openhands", sfSymbol: "bubble.and.pencil",
            brandTint: Color(red: 0xFF/255, green: 0x6B/255, blue: 0xA1/255),
            toolKey: "openhands",
            item: ItemInfo(
                description: "`~/.openhands/` stores OpenHands conversation history, sandbox file mounts, and runtime state. The sandbox file_store is recreated on next run.",
                safetyNote: "Conversation history is unrecoverable once deleted. Pulled runtime Docker images live in Docker, not here, and are not affected.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            action: .trash, tier: .low, aggregation: .none,
            notes: "Claude Code shell-session snapshots.",
            iconAsset: "claude-code", sfSymbol: "terminal",
            brandTint: Color(red: 0xD9/255, green: 0x77/255, blue: 0x57/255),
            toolKey: "claude-code",
            item: ItemInfo(
                description: "`~/.claude/shell-snapshots/` holds shell snapshots Claude Code captures during terminal sessions for diagnostic and replay purposes. Distinct from `~/.claude/projects/`, which holds the chat transcripts.",
        ...
        Rule(
            ...
            action: .trash, tier: .low, aggregation: .none,
            notes: "Claude Desktop Electron HTTP and asset cache.",
            iconAsset: "claude-code", sfSymbol: "bubble.left.and.bubble.right",
            brandTint: Color(red: 0xD9/255, green: 0x77/255, blue: 0x57/255),
            toolKey: "claude-desktop",
            item: ItemInfo(
                description: "`~/Library/Caches/com.anthropic.claudefordesktop/` is Claude Desktop's Electron HTTP and asset cache (`Cache.db`, `fsCachedData`).",
    ...
    private static let ide: [Rule] = [
        ...
        Rule(
            ...
            toolKey: "jetbrains",
            item: ItemInfo(
                description: "`~/Library/Caches/JetBrains/<product version>/` stores symbol indexes, Virtual File System snapshots, and compiled class data each JetBrains IDE builds from project sources.",
                safetyNote: "The IDE rebuilds its symbol index and VFS snapshot on the next launch. No project source files are stored here.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "jetbrains",
            item: ItemInfo(
                description: "`~/Library/Logs/JetBrains/<product version>/` stores diagnostic logs, thread dumps, and internal event traces written by each JetBrains IDE.",
                safetyNote: "Diagnostic logs only — IDE settings, project indexes, and installed plugins are unaffected.",
                regenCommand: nil,
                links: [
                    InfoLink(title: "JetBrains — IDE directories", url: "https://www.jetbrains.com/help/idea/directories-used-by-the-ide-to-store-settings-caches-plugins-and-logs.html", kind: .docs),
                ]
        ...
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`CachedExtensions/` stores serialized extension manifests and contribution-point data so the extension host starts without re-parsing each extension's `package.json`.",
        ...
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`CachedData/` contains V8 compiled bytecode for VS Code's JavaScript bundles, keyed per Electron version. Each update creates a new subdirectory; stale entries from prior versions accumulate.",
                safetyNote: "VS Code recompiles and repopulates this cache on the next launch — only startup time is affected.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`Cache/` is VS Code's Chromium HTTP response cache, holding network resources fetched by the renderer — extension marketplace data and webview content loaded by extensions.",
        ...
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`Code Cache/` holds Chromium's byte-compiled JavaScript and WebAssembly, separate from the HTTP response cache. Cached scripts load without reparsing on subsequent launches.",
                safetyNote: "Chromium recompiles scripts and repopulates this cache on the next launch — only first-launch performance is affected.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`GPUCache/` stores Chromium's compiled GPU shader programs and graphics pipeline state. Caching shaders avoids stalling the GPU on recompile each launch.",
                safetyNote: "Chromium recompiles shaders and rebuilds this cache on the next launch. No user data is stored here.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`logs/` contains timestamped subdirectories written by VS Code's main process, extension host, renderer, and language server clients. Each launch creates a new session directory; old sessions accumulate.",
                safetyNote: "Diagnostic logs only — installed extensions, settings, and workspace data are unaffected.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`Crashpad/` stores `.dmp` minidump files generated when VS Code or its renderer processes crash. Used only for post-mortem analysis.",
                safetyNote: "Crash diagnostic files only — VS Code's operation and settings are unaffected.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`workspaceStorage/` holds one UUID-named subdirectory per workspace, containing open editor tabs, scroll positions, breakpoints, and data written by extensions. AI chat extensions (GitHub Copilot Chat, Cline, …) persist conversation logs here.",
                safetyNote: "Deletes local AI chat history and editor state for that workspace — only remove if anything important has been archived.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`History/` stores per-file save snapshots recorded by VS Code's Local History feature. Snapshots are browsable from the Timeline view and provide file-level undo history that persists across editor restarts.",
                safetyNote: "Permanently deletes all saved file snapshots — only remove if no previous file version is needed.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`CachedExtensions/` in VS Code Insiders' support directory mirrors the stable channel layout, caching extension manifests and contribution-point data independently.",
        ...
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`CachedData/` in Insiders holds V8 bytecode separate from the stable channel. Daily builds with changing Electron versions cause stale subdirectories to accumulate faster than in stable.",
                safetyNote: "VS Code Insiders recompiles and repopulates this cache on the next launch — only startup time is affected.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`Cache/` in VS Code Insiders' support directory holds Chromium HTTP responses independently of the stable channel — extension marketplace requests and webview content.",
        ...
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`logs/` in VS Code Insiders holds timestamped session directories from the main process, extension host, and renderer. Daily updates cause session directories to accumulate quickly.",
                safetyNote: "Diagnostic logs only — installed extensions, settings, and workspace data are unaffected.",
                regenCommand: nil,
                links: []
            )
        ...
        Rule(
            ...
            toolKey: "cursor",
            item: ItemInfo(
                description: "`CachedData/` in Cursor's support directory holds V8 compiled bytecode using the same layout as VS Code, keyed per Electron version. Entries accumulate as Cursor updates ship new builds.",
        ...
        Rule(
            ...
            toolKey: "cursor",
            item: ItemInfo(
                description: "`CachedExtensions/` stores serialized extension manifests and contribution-point data so Cursor's extension host starts without re-parsing each `package.json`.",
        ...
        Rule(
            ...
            toolKey: "cursor",
            item: ItemInfo(
                description: "`Cache/` in Cursor's support directory holds Chromium HTTP responses for extension marketplace data and webview content.",
        ...
        Rule(
            ...
            toolKey: "cursor",
            item: ItemInfo(
                description: "`logs/` in Cursor's support directory holds timestamped session directories from the main process, extension host, renderer, and AI subsystem.",
                safetyNote: "Diagnostic logs only — installed extensions, settings, and AI chat history are unaffected.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "cursor",
            item: ItemInfo(
                description: "`workspaceStorage/` holds one UUID-named subdirectory per workspace, containing editor state (open tabs, positions) and the local log of Cursor's AI conversations.",
                safetyNote: "Deletes local AI conversation history for that workspace — only remove if anything important has been archived.",
                regenCommand: nil,
                links: []
            )
        ),
        ...
        Rule(
            ...
            toolKey: "zed",
            item: ItemInfo(
                description: "`~/Library/Caches/Zed` stores crash handler working directories and other transient data Zed writes outside its Application Support directory.",
        ...
        Rule(
            ...
            toolKey: "zed",
            item: ItemInfo(
                description: "`extensions/work/` holds per-extension build intermediates (compiled WASM modules, in-progress downloads) written during extension installation. Completed extensions move to the parent `extensions/` directory.",
        ...
        Rule(
            ...
            toolKey: "zed",
            item: ItemInfo(
                description: "`languages/` stores language server binaries (`rust-analyzer`, `gopls`, `typescript-language-server`, …) that Zed downloads automatically the first time a matching file is opened. Each server lives in its own subdirectory.",
                safetyNote: "Zed re-downloads each language server automatically the next time a matching file is opened.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "zed",
            item: ItemInfo(
                description: "`node/` contains Zed's own Node.js installation, separate from any system Node.js, used by extensions that require a JavaScript environment.",
                safetyNote: "Zed re-downloads its bundled Node.js automatically the next time an extension that requires it is activated.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "zed",
            item: ItemInfo(
                description: "`threads/` stores each Zed AI assistant conversation as a JSON file. Threads are accessible from the AI panel and can be revisited across sessions.",
                safetyNote: "Deletes the entire local Zed AI conversation history — only remove if anything important has been archived.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "sublimetext",
            item: ItemInfo(
                description: "`~/Library/Caches/com.sublimetext.4/` stores compiled syntax definitions, package metadata, and derived content built from installed Sublime Text packages.",
                safetyNote: "Sublime Text rebuilds compiled syntax definitions and package metadata on the next launch.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "sublimetext",
            item: ItemInfo(
                description: "`Index/` stores the symbol and file index that powers Go To Anything, Go To Symbol, and Go To Definition. Built by scanning open project directories; entries accumulate for every project ever opened.",
                safetyNote: "Sublime Text rebuilds the index by re-scanning open project directories. Go To Anything is slower until the index is complete.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "nova",
            item: ItemInfo(
                description: "`~/Library/Caches/com.panic.Nova` holds transient UI state, extension-derived data, and other content Nova rebuilds from project sources and extensions on demand.",
                safetyNote: "Nova rebuilds this cache on the next launch. No project files or extension settings are stored here.",
                regenCommand: nil,
                links: []
            )
        ),
        ...
        Rule(
            ...
            toolKey: "xcode",
            item: ItemInfo(
                description: "`~/Library/Caches/org.swift.swiftpm/` stores remote Swift package repository clones Xcode reuses across dependency resolutions instead of re-fetching from the network.",
                safetyNote: "Xcode re-clones required Swift package repositories on the next dependency resolution. No project source files are stored here.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "xcode",
            item: ItemInfo(
                description: "`~/Library/Caches/com.apple.dt.Xcode` stores source editor symbol indexes, module-map caches, and other derived data powering code completion, syntax highlighting, and Jump to Definition. Separate from per-project `DerivedData`; holds IDE-wide state.",
                safetyNote: "No source code lives here. Xcode rebuilds these caches from project sources on the next project open, and indexing resumes automatically.",
                regenCommand: nil,
                links: []
            )
        ),
        ...
        Rule(
            ...
            toolKey: "neovim",
            item: ItemInfo(
                description: "`~/.cache/nvim` stores log files, LSP protocol traces, Tree-sitter compiled parser libraries (`.so` files), and plugin-specific cache data.",
        ...
        Rule(
            ...
            toolKey: "neovim",
            item: ItemInfo(
                description: "`~/.local/share/nvim/swap/` stores `*.swp` swap files for each open buffer. If Neovim is killed or crashes, these enable recovery of unsaved changes on next open.",
                safetyNote: "Swap files from a cleanly closed session contain no unsaved changes — safe to delete when Neovim is not running.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "neovim",
            item: ItemInfo(
                description: "`~/.local/share/nvim/mason/packages/` stores LSP servers, DAP adapters, linters, and formatters installed by Mason.nvim. Each tool occupies its own subdirectory, exposed to Neovim via PATH shims.",
                safetyNote: "Reinstall tools via `:MasonInstall <name>`. Language features are unavailable until reinstalled.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "emacs",
            item: ItemInfo(
                description: "`~/.emacs.d/eln-cache/` stores Elisp packages compiled to native machine code (`.eln` files) by Emacs 28+'s native compilation feature.",
                safetyNote: "Emacs recompiles all Elisp packages to native code on the next startup. Startup is slower until background compilation completes.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "emacs",
            item: ItemInfo(
                description: "`~/.emacs.d/.local/cache/` stores Doom Emacs package metadata, compiled autoload files, and other derived artifacts. Managed by `doom sync`.",
        ...
        Rule(
            ...
            toolKey: "helix",
            item: ItemInfo(
                description: "`~/.cache/helix/` stores Helix's log file, recording startup events, LSP communication traces, and runtime warnings.",
                safetyNote: "Log files only — Helix configuration, themes, and LSP operation are unaffected.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            languageKey: "rust",
            item: ItemInfo(
                description: "`~/.cache/rust-analyzer/` holds rust-analyzer's incremental analysis database — proc-macro expansion results, Salsa query cache, and type-checking outputs — for resuming analysis after a restart.",
                safetyNote: "rust-analyzer rebuilds its analysis database from source on the next editor session. IntelliSense is slower until indexing completes.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            item: ItemInfo(
                ...
                safetyNote: "jdtls rebuilds its workspace index from project source and class files on the next editor session. Java IntelliSense is slower until indexing completes.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            item: ItemInfo(
                description: "`~/.config/coc/extensions/node_modules/` stores each coc.nvim extension's `node_modules` subtree. coc.nvim loads VS Code-compatible extensions written in Node.js.",
                safetyNote: "Extensions must be reinstalled via `:CocInstall <name>`. Language-server features are unavailable until then.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            item: ItemInfo(
                ...
                safetyNote: "SourceKit-LSP rebuilds the source index from build artifacts on the next editor session. Swift IntelliSense is slower until indexing completes.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            item: ItemInfo(
                description: "`User/globalStorage/github.copilot/` holds GitHub Copilot's completion telemetry queue and model cache for the VS Code extension.",
        ...
        Rule(
            ...
            toolKey: "github-copilot",
            item: ItemInfo(
                description: "`User/globalStorage/github.copilot-chat/` stores Copilot Chat's local workspace embeddings index and cached chat transcripts. Chat history syncs to GitHub, so the local copy is rebuildable.",
                safetyNote: "Workspace index re-indexes on demand. Chat transcripts re-sync from github.com on sign-in.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            item: ItemInfo(
                ...
                safetyNote: "Refetched from the GitHub API on the next extension activation.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`User/globalStorage/eamodio.gitlens/` caches GitLens' blame metadata and contributor avatars across opened repositories.",
        ...
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`Code/CachedData/` is a peer of `Code Cache/` — VS Code keeps a per-commit V8 bytecode cache here. Accumulates across editor upgrades.",
        ...
        Rule(
            ...
            toolKey: "vscode",
            item: ItemInfo(
                description: "`Code/CachedExtensionVSIXs/` keeps downloaded `.vsix` archives for installed extensions. Used by the extension manager for reinstalls and repairs.",
        ...
        Rule(
            ...
            toolKey: "jetbrains",
            item: ItemInfo(
                description: "Within each `~/Library/Caches/JetBrains/<IDE><Year>.<X>/` directory, the `index/` subtree holds per-project symbol and stub indexes. These are re-derived from sources but the rebuild is slow.",
        ...
        Rule(
            ...
            toolKey: "jetbrains",
            item: ItemInfo(
                description: "Per-IDE `event-log-data/` directories under `~/Library/Application Support/JetBrains/` queue Feature Usage Statistics telemetry payloads until shipped or expired.",
        ...
        Rule(
            ...
            toolKey: "jetbrains",
            item: ItemInfo(
                description: "Per-IDE `plugins-sandbox/` directories hold the sandboxed IDE installs spun up by the Gradle IntelliJ Plugin during plugin development. Each sandbox is a self-contained IDE clone.",
                safetyNote: "Rebuilt by the next `runIde` or `buildPlugin` Gradle task.",
                regenCommand: nil,
                links: []
            )
        ),
        Rule(
            ...
            toolKey: "jetbrains",
            item: ItemInfo(
                description: "Per-IDE `log-archive/` and similar rolled-log directories under the JetBrains caches tree.",
                safetyNote: "Diagnostic-only; recreated on the next launch.",
                regenCommand: nil,
                links: []
            )
        ),
    ...
    private static let devops: [Rule] = [
        Rule(
            ...
            toolKey: "pulumi",
            item: ItemInfo(
                description: "`~/.pulumi/plugins` holds Pulumi's downloaded resource-provider plugins, one directory per provider+version. Pulumi keeps every version it has ever installed.",
                safetyNote: "Pulumi re-downloads needed plugins on the next `pulumi up` (or via `pulumi plugin install`). The companion `~/.pulumi/templates/` holds project templates, refilled via `pulumi new`.",
        ...
        Rule(
            ...
            toolKey: "terraform",
            item: ItemInfo(
                description: "`~/.terraform.d/plugin-cache` is the shared provider-plugin cache Terraform projects pull from instead of each downloading their own copy. Only populated when `plugin_cache_dir` is set in `~/.terraformrc`.",
        ...
        Rule(
            ...
            action: .trash, tier: .medium, aggregation: .none,
            notes: "Cached chart `.tgz` archives and repo `index.yaml` files.",
            iconAsset: "helm",
            brandTint: helmBlue,
            toolKey: "helm",
            item: ItemInfo(
                description: "`~/Library/Caches/helm/repository` holds downloaded chart archives (`.tgz`) and `<repo>-index.yaml` metadata for every Helm repository the user has added.",
                safetyNote: "`helm repo update` refreshes the index files. Charts are re-downloaded on `helm pull` or `helm install` as needed.",
        ...
        Rule(
            ...
            toolKey: "kubectl",
            item: ItemInfo(
                description: "`~/.kube/cache/discovery/` caches API-discovery documents (`servergroups.json`, `serverresources.json`) per cluster API endpoint. One tree per server kubectl has talked to; default TTL is 10 minutes.",
                safetyNote: "Refilled on the next `kubectl` call.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "kubectl",
            item: ItemInfo(
                description: "`~/.kube/cache/http/` is kubectl's ETag-keyed HTTP response cache, used to short-circuit unchanged discovery responses.",
                safetyNote: "Refilled on the next `kubectl` call.",
                regenCommand: nil,
                links: []
            )
        ...
        Rule(
            ...
            toolKey: "gcloud",
            item: ItemInfo(
                description: "`~/.config/gcloud/logs/` collects per-invocation gcloud CLI logs in dated subdirectories (`YYYY.MM.DD/HH.MM.SS.NNNNNN.log`). Accumulates without rotation.",
                safetyNote: "Diagnostic-only; written fresh on every `gcloud` call.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "gh",
            item: ItemInfo(
                description: "`~/Library/Caches/com.github.gh` is the GitHub CLI's HTTP cache layer for `gh api`, `gh pr`, and `gh repo` calls. Some commands use the cache implicitly; others opt in via `--cache`.",
                safetyNote: "Refilled on the next `gh` call.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "act",
            item: ItemInfo(
                description: "`~/.cache/act/` holds act's cached runner images, action source checkouts, and artifact-server payloads from running GitHub Actions locally.",
    ...
    private static let gameDev: [Rule] = [
        ...
        Rule(
            ...
            toolKey: "unity",
            item: ItemInfo(
                description: "`~/Library/Unity/cache/` holds packages downloaded by Unity's Package Manager (UPM) for use across projects, alongside per-user Unity working data.",
        ...
        Rule(
            ...
            action: .trash, tier: .high, aggregation: .none,
            notes: "Downloaded Asset Store packages — re-downloadable from your library.",
            iconAsset: "unity",
            brandTint: unityBlack,
            toolKey: "unity",
            item: ItemInfo(
                description: "`~/Library/Unity/Asset Store-5.x/` is where the Unity Editor caches `.unitypackage` archives downloaded from the Asset Store.",
                safetyNote: "Re-downloadable from your Asset Store library (purchased entitlements persist on Unity's side). Free assets only stay re-downloadable while their listings remain on the store — verify before clearing if a publisher may have delisted.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            action: .trash, tier: .medium, aggregation: .none,
            notes: "Engine-wide DDC — cooked assets and compiled shaders.",
            iconAsset: "unrealengine",
            brandTint: unrealBlack,
            toolKey: "unreal",
            item: ItemInfo(
                description: "`~/Library/Application Support/Epic/UnrealEngine/Common/DerivedDataCache/` is Unreal's engine-wide DDC — derived versions of source assets (cooked textures, light maps, …) and compiled shaders, shared across projects on this machine. Per-project DDC may also live alongside the project's `.uproject`.",
                safetyNote: "Unreal rebuilds DDC entries on demand the next time you open a project or trigger a cook. First build after clearing is slow but everything is reproducible from sources.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "godot",
            item: ItemInfo(
                description: "`~/Library/Caches/Godot/` is Godot's user-level editor cache — recent-projects metadata, thumbnails, and other transient data the editor regenerates on demand. Per-project shader and import caches live in each project's `.godot/` directory.",
        ...
        Rule(
            ...
            toolKey: "godot",
            item: ItemInfo(
                description: "`~/Library/Application Support/Godot/shader_cache/` holds the editor's compiled shader binaries (used by previews and editor rendering). Per-project runtime shader caches in each project's `.godot/shader_cache/` are not touched.",
        ...
        Rule(
            ...
            action: .trash, tier: .low, aggregation: .none,
            notes: "macOS Metal AIR cache for non-bundled binaries (cargo, swift run, scripts).",
            iconAsset: "metal",
            brandTint: metalSilver,
            languageKey: "swift",
            toolKey: "metal",
            item: ItemInfo(
                description: "`$DARWIN_USER_CACHE_DIR/com.apple.metal/` (resolves to `/private/var/folders/<X>/<Y>/C/com.apple.metal/`) is the catch-all Metal binary shader cache for processes without a bundle ID — `cargo run` builds, `swift run` executables, ad-hoc Python scripts hitting Metal via PyTorch / MLX. Bundled apps get their own `<bundle-id>/com.apple.metal/` peer subdirectories.",
                safetyNote: "macOS rebuilds the cache on the next render — only a perf hit while it warms back up. Does not affect WGPU's own pipeline cache or MoltenVK's per-app pipeline blob.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "metal",
            item: ItemInfo(
                description: "`$DARWIN_USER_CACHE_DIR/com.apple.metalfe/` caches compiled shaders for MetalFX (Apple's spatial and temporal upscaling framework used by some games and rendering apps). Same per-user-session cache root as `com.apple.metal/`.",
                safetyNote: "Recompiled on demand by MetalFX's first invocation in each app.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "metal",
            item: ItemInfo(
                description: "Each bundled macOS app that compiles Metal shaders gets its own subdirectory under `$DARWIN_USER_CACHE_DIR/<bundle-id>/com.apple.metal/`. One finding per app whose cache exists, so heavy GPU users (LM Studio, OBS, video editors, AI image generators) can be cleaned individually. The top-level `com.apple.metal/` for non-bundled binaries is catalogued separately via `metal.global-cache`.",
        ...
        Rule(
            ...
            toolKey: "metal",
            item: ItemInfo(
                description: "Per-app MetalFX shader caches — sibling of `metal.per-app-cache` for apps using Apple's spatial and temporal upscaling framework. One finding per `<bundle-id>/com.apple.metalfe/` directory.",
                safetyNote: "Recompiled on demand by MetalFX's first invocation in each app. Perf hit only.",
                regenCommand: nil,
                links: [
                    InfoLink(title: "MetalFX — Apple Developer", url: "https://developer.apple.com/documentation/metalfx", kind: .docs),
                ]
        ...
        Rule(
            ...
            toolKey: "electron",
            item: ItemInfo(
                description: "`~/Library/Application Support/<app>/Cache/` is the Chromium HTTP cache every Electron-based desktop app inherits (Discord, Slack, Linear, Notion, Figma, Postman, Bruno, …). Some native macOS apps use the same directory name for HTTP caching — equally safe to clear.",
        ...
        Rule(
            ...
            toolKey: "electron",
            item: ItemInfo(
                description: "`~/Library/Application Support/<app>/Code Cache/` is V8's compiled-JavaScript bytecode cache (`js/`, `wasm/`). Chromium-specific — found in Electron apps and CEF-based hosts.",
                safetyNote: "Auto-rebuilt by V8 on next app launch — only a marginal slowdown while it recompiles.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "electron",
            item: ItemInfo(
                description: "`~/Library/Application Support/<app>/GPUCache/` is the Chromium-level GPU shader/driver cache used by Electron apps. Distinct from the underlying Metal AIR cache the same renderer process also writes — different layer, both regenerable. Sibling caches `DawnGraphiteCache/` and `DawnWebGPUCache/` are catalogued separately.",
        ...
        Rule(
            ...
            toolKey: "electron",
            item: ItemInfo(
                description: "`~/Library/Application Support/<app>/DawnGraphiteCache/` caches compiled pipelines for Dawn — Chromium's WebGPU implementation — on Apple's Graphite (Skia GPU) backend. Added in Chromium 115; visible in apps shipping recent Electron versions.",
        ...
        Rule(
            ...
            toolKey: "electron",
            item: ItemInfo(
                description: "`~/Library/Application Support/<app>/DawnWebGPUCache/` caches compiled pipelines for Dawn-backed WebGPU usage in Electron apps. Added in Chromium 113; populated by apps using the WebGPU API (some AI inference UIs and 3D viewers).",
        ...
        Rule(
            ...
            toolKey: "electron",
            item: ItemInfo(
                description: "`~/Library/Application Support/<app>/Service Worker/CacheStorage/` is the on-disk store backing the web platform's `caches` API. Apps that aggressively pre-cache assets via a Service Worker (Notion, Linear, Outlook, …) can accumulate large amounts here. Distinct from `IndexedDB/` and `Local Storage/`, which contain user state and are not touched.",
                safetyNote: "Re-fetched from origin on the next app launch. The companion `Service Worker/Database/` (registration metadata) is not touched.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "electron",
            item: ItemInfo(
                description: "`~/Library/Application Support/<app>/Crashpad/` is Chromium's crash-dump database — `.dmp` minidumps from past crashes plus upload bookkeeping. Useful only to the app's developers.",
                safetyNote: "Recreated on the next crash. Does not affect macOS-level crash reports under `~/Library/Logs/DiagnosticReports/` (separate rule).",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "webkit",
            item: ItemInfo(
                description: "`~/Library/Caches/<bundle>/WebKit/NetworkCache/Version <N>/` is WKWebView's HTTP response cache for any app embedding the WebKit view — Tauri apps, Webcatalog wrappers, and native macOS apps that use WKWebView for content panes (Sketch, Cyberduck, OBS Studio's stats overlay, …).",
                safetyNote: "Re-fetched on next request — only a perf hit while the cache rebuilds.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "webkit",
            item: ItemInfo(
                description: "`~/Library/Caches/<bundle>/WebKit/CacheStorage/<origin-hash>/` backs the web platform's `caches` API for WKWebView-hosted content. PWA-style web apps wrapped in Tauri / Webcatalog / Safari Web Apps can accumulate large amounts here as they pre-cache offline assets.",
                safetyNote: "Re-fetched from origin on next page load. Distinct from `WebsiteData/IndexedDB/` and `WebsiteData/LocalStorage/`, which contain user data and are not touched.",
                regenCommand: nil,
                links: [
                    InfoLink(title: "Cache API — MDN", url: "https://developer.mozilla.org/en-US/docs/Web/API/Cache", kind: .docs),
                ]
        ...
        Rule(
            ...
            toolKey: "webkit",
            item: ItemInfo(
                description: "`~/Library/Caches/<bundle>/WebKit/OfflineWebApplicationCache/` is the deprecated HTML5 Application Cache (`<html manifest=…>`) store. The directory is still created lazily by WebKit and lingers from older site-version manifests.",
                safetyNote: "The spec is removed from modern browsers; if a site still relies on it, the manifest is re-fetched on next load.",
                regenCommand: nil,
                links: [
        ...
        Rule(
            ...
            toolKey: "webkit",
            item: ItemInfo(
                description: "`~/Library/Caches/<bundle>/WebKit/ServiceWorkers/Scripts/` caches the JavaScript bodies of registered Service Workers. The companion registration database (`Database/`) at the same level holds the actual SW lifecycle state and is not touched.",
                safetyNote: "Re-fetched from origin on the next Service Worker activation.",
                regenCommand: nil,
                links: [
                    InfoLink(title: "Service Worker API — MDN", url: "https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API", kind: .docs),
                ]
        ...
        Rule(
            ...
            toolKey: "sparkle",
            item: ItemInfo(
                description: "`~/Library/Caches/<bundle-id>/org.sparkle-project.Sparkle/` is where the Sparkle auto-update framework stages downloaded `.zip`/`.dmg` archives, EdDSA signature blobs, and extraction intermediates. Sparkle ships in a wide range of macOS indie apps (Cyberduck, Transmission, Postico, OBS Studio, OpenEmu, DaisyDisk, MacVim, BBEdit, …); the staging area persists after install and is never auto-cleaned.",
                safetyNote: "Sparkle re-downloads the archive next time it has a new update to install. The installed app is unaffected.",
                regenCommand: nil,
                links: [
    ...
    private static let database: [Rule] = {
        ...
        rules.append(Rule(
            ...
            action: .trash, tier: .extreme, aggregation: .none,
            notes: "Homebrew PostgreSQL data directory — every database on this server.",
            iconAsset: "postgresql",
            sfSymbol: "cylinder.fill",
            brandTint: postgresBlue,
            toolKey: "postgres",
            item: ItemInfo(
                description: "`/opt/homebrew/var/postgresql@<N>` is the data directory (`PGDATA`) for a Homebrew-installed PostgreSQL major version. Holds every database on the instance: `base/<oid>/` per database, `pg_wal/` (write-ahead log), `pg_xact/` (transaction commit status), plus config (`postgresql.conf`, `pg_hba.conf`). Multiple major versions can coexist; each is surfaced separately.",
                safetyNote: "**Deletion is irreversible.** Every database on this server will be lost. Stop the service first (`brew services stop postgresql@<N>`), `pg_dumpall` anything worth keeping, then `initdb` to start fresh.",
        ...
        rules.append(Rule(
            ...
            toolKey: "postgres",
            item: ItemInfo(
                description: "`~/Library/Application Support/Postgres/var-<N>/` is one Postgres.app server cluster — a PostgreSQL data directory (`PGDATA`) equivalent to `/opt/homebrew/var/postgresql@N/`. Postgres.app manages each major version separately so multiple PG versions can run concurrently.",
                safetyNote: "**Deletion is irreversible.** Every database in this server cluster will be lost. Use Postgres.app's UI to dump or migrate first; the app re-creates the per-version directory on next launch.",
                regenCommand: nil,
                links: [
        ...
        rules.append(Rule(
            ...
            toolKey: "mysql",
            item: ItemInfo(
                description: "`/opt/homebrew/var/mysql<suffix>/` is a MySQL data directory (`datadir`). The unsuffixed form is the default `mysql` formula; suffixed forms (`mysql@5.7/`, `mysql@8.0/`, `mysql@8.4/`) come from version-pinned formulas. Contains the system tablespace (`ibdata1`), per-schema `.ibd` tablespace files, the InnoDB redo log (`ib_logfile*`), the binary log (`mysql-bin.*`), and configuration metadata.",
                safetyNote: "**Deletion is irreversible.** Every schema, including `mysql.user`, will be lost. Stop the server (`brew services stop mysql<@N>`), `mysqldump --all-databases` anything worth keeping, then `mysqld --initialize` to start fresh.",
                regenCommand: nil,
                links: [
        ...
        rules.append(Rule(
            ...
            toolKey: "mariadb",
            item: ItemInfo(
                description: "`/opt/homebrew/var/mariadb` is the MariaDB data directory. Layout matches MySQL's (`ibdata1`, per-schema `.ibd` files, redo log, binary log).",
        ...
        rules.append(Rule(
            ...
            action: .trash, tier: .extreme, aggregation: .none,
            notes: "Homebrew Redis persistence — RDB snapshot and AOF append-only log.",
            iconAsset: "redis",
            sfSymbol: "cylinder.fill",
            brandTint: redisRed,
            toolKey: "redis",
            item: ItemInfo(
                description: "`/opt/homebrew/var/db/redis` holds Redis's persistence files: `dump.rdb` (point-in-time snapshot, written periodically per `save` thresholds) and/or `appendonly.aof` (every write since the last rewrite, replayed on startup). One or both may be present depending on `redis.conf`.",
                safetyNote: "**Deletion is irreversible** if persistence is the only copy of the data. Stop the server (`brew services stop redis`), `BGSAVE` first if a final snapshot is wanted, then delete. The next service boot starts with an empty dataset.",
                regenCommand: nil,
                links: [
        ...
        rules.append(Rule(
            ...
            toolKey: "mongodb",
            item: ItemInfo(
                description: "`/opt/homebrew/var/mongodb<suffix>/` is the MongoDB data path (`storage.dbPath`). Unsuffixed is the default `mongodb-community` formula; suffixed (`@5.0`, `@6.0`, `@7.0`, …) come from version-pinned formulas. With the default WiredTiger engine, holds the WiredTiger journal, collection and index `.wt` files, storage engine metadata (`WiredTiger.wt`, `WiredTiger.turtle`), and the rotated `diagnostic.data/` server-stat archive.",
        ...
        rules.append(Rule(
            ...
            item: ItemInfo(
                description: "`/opt/homebrew/var/lib/elasticsearch/` (`path.data` in `elasticsearch.yml`) holds every Lucene index segment, the cluster state, and per-node translog files. Each index lives under `nodes/<n>/indices/<uuid>/` with a directory per shard.",
                safetyNote: "**Deletion is irreversible.** All indexed documents on this node are lost. Take a snapshot first (`PUT _snapshot/<repo>/<name>`) for anything that can't be reindexed from the source of truth.",
                regenCommand: nil,
                links: [
        ...
        rules.append(Rule(
            ...
            toolKey: "kafka",
            item: ItemInfo(
                description: "`/opt/homebrew/var/lib/{kafka,kraft-combined}-logs/` is Kafka's `log.dirs` location — one subdirectory per topic-partition, each with `.log` segment files (the messages), `.index` offset indices, and `.timeindex` time-based indices. Despite the name, this is the message store, not a diagnostic log. `kafka-logs` is the legacy ZooKeeper-mode layout; `kraft-combined-logs` is the KRaft-mode (Kafka 3.3+) layout combining broker and controller state.",
                safetyNote: "**Deletion is irreversible.** All topic data is lost. Stop the broker (`brew services stop kafka`); migrate via MirrorMaker or `kafka-console-consumer` + reproducer first if needed.",
                regenCommand: nil,
                links: [
        ...
        rules.append(Rule(
            ...
            toolKey: "zookeeper",
            item: ItemInfo(
                description: "`/opt/homebrew/var/run/zookeeper/data/` is ZooKeeper's `dataDir` — `version-2/snapshot.<zxid>` files (point-in-time tree snapshots) and `version-2/log.<zxid>` files (the transaction log replayed on restart). Often paired with a Kafka broker for older Kafka versions; KRaft-mode Kafka (3.3+) does not need it.",
                safetyNote: "**Deletion is irreversible.** Cluster state, ACLs, and ephemeral nodes are lost. If this is a Kafka coordinator, the broker may not boot until ZooKeeper has been re-initialized.",
                regenCommand: nil,
                links: [
        ...
        rules.append(Rule(
            ...
            toolKey: "postgres",
            item: ItemInfo(
                description: "`/opt/homebrew/var/log/postgresql@<N>.log` is the brew-services-managed log for PostgreSQL <N>. Captures stderr from `postgres` (startup messages, slow queries if logged, autovacuum, errors). Rotates only on service restart, so it accumulates indefinitely.",
                safetyNote: "Re-created on the next `brew services restart postgresql@<N>`. A running server may hold an open handle, in which case the file truncates rather than disappearing.",
                regenCommand: nil,
                links: []
            )
        ))
        ...
        rules.append(Rule(
            ...
            toolKey: "postgres",
            item: ItemInfo(
                description: "`/opt/local/var/db/postgresql<N>/` (typically with `defaultdb/` inside) is the data root for a MacPorts-installed PostgreSQL major version. Layout matches Homebrew's `PGDATA` — `base/<oid>/`, `pg_wal/`, `pg_xact/`, config files. The `defaultdb/` subdir is the actual `PGDATA`; the parent holds version metadata.",
                safetyNote: "**Deletion is irreversible.** The entire version directory is deleted under sudo. Stop the service first (`sudo port unload postgresql<N>-server`), `pg_dumpall` anything worth keeping, then re-run the postgresql<N>-server's port-provided init script.",
                regenCommand: nil,
                links: [
        ...
        rules.append(Rule(
            ...
            toolKey: "mysql",
            item: ItemInfo(
                description: "`/opt/local/var/db/mysql<N>/` is the MacPorts MySQL `datadir`. Layout matches Homebrew's MySQL: `ibdata1`, per-schema `.ibd` files, redo log, binary log.",
                safetyNote: "**Deletion is irreversible.** Stop the server (`sudo port unload mysql<N>-server`), `mysqldump --all-databases` first, then run the port's init script.",
                regenCommand: nil,
                links: [
                    InfoLink(title: "MacPorts Guide — port", url: "https://guide.macports.org/", kind: .docs),
                    InfoLink(title: "MySQL — Data Directory", url: "https://dev.mysql.com/doc/refman/8.0/en/data-directory.html", kind: .docs),
        ...
        rules.append(Rule(
            ...
            toolKey: "mariadb",
            item: ItemInfo(
                description: "`/opt/local/var/db/mariadb-<X>.<Y>/` is the MacPorts MariaDB datadir. Layout matches MySQL's.",
        ...
        rules.append(Rule(
            ...
            toolKey: "redis",
            item: ItemInfo(
                description: "`/opt/local/var/db/redis<suffix>/` holds Redis's persistence files (`dump.rdb` snapshot, `appendonly.aof`). Layout matches Homebrew's `/opt/homebrew/var/db/redis/`. Catches the unsuffixed `redis` port plus the `redis-sentinel` variant.",
        ...
        rules.append(Rule(
            ...
            toolKey: "mongodb",
            item: ItemInfo(
                description: "`/opt/local/var/db/mongodb<NN>/` is the MongoDB `storage.dbPath` — WiredTiger journal, `.wt` collection and index files, rotated `diagnostic.data/`. Multiple major versions can coexist via separate ports (`mongodb50`, `mongodb60`, …); each is surfaced separately.",
        ...
        rules.append(Rule(
            ...
            toolKey: "dbngin",
            item: ItemInfo(
                description: "`~/Library/Application Support/com.tinyapp.dbnginapp/<service>/` is one DBngin service-type folder (`postgres/`, `mysql/`, `redis/`, `mongodb/`). Inside is one subdirectory per version DBngin has downloaded (`16.0/`, `8.0.32/`, …), each containing server binaries plus a `data/` directory with the actual database files.",
                safetyNote: "**Deletion is irreversible.** All databases across every version DBngin tracks for this service will be lost. Stop every instance of this engine from the app, export with the per-DB tooling (`pg_dumpall`, `mysqldump`, `mongodump`, `BGSAVE`+copy `dump.rdb`), then delete. DBngin re-creates an empty per-version folder on next spin-up.",
                regenCommand: nil,
                links: [
        ...
        rules.append(Rule(
            ...
            action: .trash, tier: .low, aggregation: .none,
            notes: "Postico 2 query plans, schema introspection, and syntax-highlighting caches.",
            iconAsset: "postico",
            sfSymbol: "tablecells.fill",
            brandTint: posticoBlue,
            toolKey: "postico",
            item: ItemInfo(
                description: "`~/Library/Caches/at.eggerapps.Postico/` is Postico 2's standard macOS cache — query-plan caches, schema introspection results, syntax-highlighting tables. Connection profiles and saved queries live in `~/Library/Application Support/at.eggerapps.Postico/` and are not touched.",
                safetyNote: "Postico 2 rebuilds the cache on demand. First connection to each server reintrospects the schema — a one-time delay.",
                regenCommand: nil,
                links: [
        ...
        rules.append(Rule(
            ...
            toolKey: "tableplus",
            item: ItemInfo(
                description: "`~/Library/Caches/com.tinyapp.TablePlus/` is TablePlus's standard macOS cache — schema introspection, completion indexes, query result caches. Connection profiles live in `~/Library/Application Support/com.tinyapp.TablePlus/` and are not touched.",
                safetyNote: "TablePlus rebuilds the cache on demand. Schema introspection runs again on first connection to each server.",
                regenCommand: nil,
                links: [
        ...
        rules.append(Rule(
            ...
            toolKey: "dbeaver",
            item: ItemInfo(
                description: "`~/Library/DBeaverData/workspace6/.metadata/` is the Eclipse-RCP workspace metadata DBeaver inherits from its IDE shell — `.log` files, indexer state, view-layout snapshots, and per-plugin runtime caches. Connection profiles live in the sibling `General/.dbeaver/` and are not touched.",
                safetyNote: "DBeaver re-creates the workspace metadata on next launch. View layouts may need a one-time re-arrangement.",
                regenCommand: nil,
                links: [
